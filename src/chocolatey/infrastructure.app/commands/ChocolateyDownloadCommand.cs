...
namespace chocolatey.infrastructure.app.commands
{
    ...
    [CommandFor("download", "downalods packages from various sources")]
    public class ChocolateyDownloadCommand : ICommand
    {
        private readonly IChocolateyPackageService _packageService;

        public ChocolateyDownloadCommand(IChocolateyPackageService packageService)
        {
        ...
        public virtual void configure_argument_parser(OptionSet optionSet, ChocolateyConfiguration configuration)
        {
            optionSet
                ...
                .Add("i|ignoredependencies|ignore-dependencies",
                     "IgnoreDependencies - Ignore dependencies when downloading package(s). Defaults to false.",
                     option => configuration.IgnoreDependencies = option != null)
                .Add("x|forcedependencies|force-dependencies",
                     "ForceDependencies - Force dependencies to be redownloaded when force downloading package(s). Must be used in conjunction with --force. Defaults to false.",
                ...
                .Add("checksumtype64=|checksumtypex64=|checksum-type-x64=|downloadchecksumtypex64=|download-checksum-type-x64=",
                 ...
                 .Add("stoponfirstfailure|stop-on-first-failure|stop-on-first-package-failure",
                     "Stop On First Package Failure - stop running download on first package failure instead of continuing with others. Overrides the default feature '{0}' set to '{1}'. Available in 0.10.4+.".format_with(ApplicationParameters.Features.StopOnFirstPackageFailure, configuration.Features.StopOnFirstPackageFailure.to_string()),
                ...
                .Add("disable-repository-optimizations|disable-package-repository-optimizations",
                     "Disable Package Repository Optimizations - Do not use optimizations for reducing bandwidth with repository queries during package download operations. Should not generally be used, unless a repository needs to support older methods of query. When used, this makes queries similar to the way they were done in Chocolatey v0.10.11 and before. Overrides the default feature '{0}' set to '{1}'. Available in 0.10.14+.".format_with
        ...
        public virtual void handle_validation(ChocolateyConfiguration configuration)
        {
            ...
            {
                throw new ApplicationException("Package name is required. Please pass at least one package name to download.");
            }
        ...
        {
            this.Log().Info(ChocolateyLoggers.Important, "download Command");
            this.Log().Info(@"
Downloads a package or a list of packages (sometimes specified as a
 packages.config).
...
");
            ...
            "chocolatey".Log().Info(@"
    choco download <pkg|packages.config> [<pkg2> <pkgN>] [<options/switches>]
    cinst <pkg|packages.config> [<pkg2> <pkgN>] [<options/switches>]

NOTE: `all` is a special package keyword that will allow you to download
...
");
    ...
    choco download notepadplusplus googlechrome atom 7zip
    choco download notepadplusplus --force --force-dependencies
    choco download notepadplusplus googlechrome atom 7zip -dvfy
    choco download nodejs --version 0.10.35
    choco download git -s ""'https://somewhere/out/there'""
    choco download git -s ""'https://somewhere/protected'"" -u user -p pass

Choco can also download directly from a nuspec/nupkg file. This aids in
 testing packages:
...
".format_with(ApplicationParameters.Features.UsePackageExitCodes, ApplicationParameters.Features.ExitOnRebootDetected));
            ...
            "chocolatey".Log().Info(@"
Chocolatey FOSS download showing tab completion and `refreshenv` (a way
...
");
        ...
        {
            _packageService.download_noop(configuration);
        }

        public virtual void run(ChocolateyConfiguration configuration)
        {
            _packageService.ensure_source_app_installed(configuration);
            _packageService.download_run(configuration);
        }
